<template>
  <div>
    ...
    <Modal
      v-model="visible"
      width="650"
      :title="$t('itemProperty.showHideEditorTitle')"
      :ok-text="$t('common.confirm')"
      @on-ok="confirm"
      :cancel-text="$t('common.cancel')">
      <p v-if="mapData.items.length === 0">{{$t('itemProperty.showHideNoRules')}}</p>
      <m-form
        v-else
        ref="mapData"
        :model="mapData"
        ...
        <m-row
          ...
          <m-col span="12">
            <m-form-item :label="$t('itemProperty.showHideDisFollowCtrls')">
      ...
      <div slot="footer">
        <m-row>
          <m-col
            ...
            <m-button
              type="success"
              :disabled="mapData.items.length >= values.length"
              icon="plus-round"
              @click="add()">
              {{$t('common.add')}}
            </m-button>
    ...
    methods: {
      edit() {
        ...
        this.$store.state.xFormData.items.forEach(item => {
          if (item.id !== this.$store.state.xFormData.activeItemId) {
            if (item.title && item.title[this.currentLang] && item.id) {
